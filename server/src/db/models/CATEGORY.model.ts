...
categorySchema.methods.updateBlogsCategory =
  async function (this: Category, newName: string) {
...
categorySchema.methods.deleteBlogsCategory =
  async function (this: Category) {
    ...
    blogs.forEach(async blog => {
      ...
      await updateCategByCategName(
        blog._id as ObjectId, '无', false);
      await updateCategByCategName(
        blog._id as ObjectId, this.categoryName, true);
...
Category.findOne({ id: '-1' }).then(res => {
  if (!res) new Category({
    id: '-1',
    categoryName: '无',
    createAt: new Date(0),
...
const updateCategById = async (
  ...
  const updatedCateg = await Category.findOneAndUpdate(
    { id }, { categoryName: newName }, { new: true });

  await updatedCateg.updateBlogsCategory(newName);

  const { id: categId, categoryName } =
    updatedCateg as { id: string, categoryName: string };
...
const deleteCategById = async (id: string) => {
  ...
  const deletedCateg = await Category.findOneAndDelete(
    { id }, { new: true });
...
export async function updateCategByCategName(
  BlogObjectId: ObjectId, categoryName: string, isRemove: boolean,
): Promise<void> {
  if (!categoryName) return;

  if (isRemove === false && categoryName !== '无') {
    const hasTag = await Category.findOne(
