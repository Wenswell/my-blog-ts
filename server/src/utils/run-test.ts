...
import { addTag, getAllTag, updateTagById, deleteTagById } from '@/db/models/TAG';
...
async function testTAG() {
  ...
  console.log('updateTagById()', await updateTagById('DLQXWO', `new at ${new Date().toString()}`));
...
async function testCATEGORY() {
  ...
  console.log('updateCategById()', await updateCategById('Zg2IulI', `new at ${new Date().toString()}`));
...
async function testBLOG() {
  ...
  const updatabyiddd = await updateBlogById('U4mnT2NqE',
    { title: `new title at${new Date().toString()}` });
  console.log('updatabyiddd', updatabyiddd);
